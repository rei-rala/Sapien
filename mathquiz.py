...
def Sapien(p):
    ...
    try: 
        ...
        p.puntos -= 1
        p.errores.append(f'Round {p.round} -> {select} = {str(respuesta)}')   # aniade los errores a la clase para mostrarlos al decidir ganador
    else:
    ...
    TABLA_OPERACIONES.pop( key_operation )  # Elimina la opcion elegida por el sistema para evitar que haya dos opciones iguales
...
for i in range(This_Game.q_players):    # crea clases en funcion de la cantidad de jugadores declarados
    time.sleep(1)
    i = Jugador( input( f'{bcolors.OKGREEN}Ingrese nombre para jugador {i+1}: {bcolors.ENDC}'), 0, 0, [], 0 )   # DEBUG -> i = Jugador( nombre=f'JUGADOR {i+1}' ), puntos=0, aciertos=0, errores=[] ) / creacion de clases con nombre personalizado
